...
# Useful variables
STUDENTS_2="fd00:200:3:301::/64"
STUDENTS_3="fd00:300:3:301::/64"
...
echo 'Setting default policies'
    ...
    # Allow BGP packets (border router)
    ip6tables -A INPUT -p tcp -m tcp --destination-port 179 -j ACCEPT
    ip6tables -A OUTPUT -p tcp -m tcp --destination-port 179 -j ACCEPT 
    ...
    do
        ...
        do
            ...
            # Allow DNS traffic (over both UDP and TCP)
            # First datacenter
    ...
    for k in 200 300;
    do
        ip6tables -A FORWARD -p tcp -s fd00:$k:3:401::/64 -d fd00:$k::3/48 --destination-port 80 -j ACCEPT
        ip6tables -A FORWARD -p tcp -s fd00:$k:3:401::/64 -d fd00:$k::3/48 --destination-port 443 -j ACCEPT;
    done

    # Block camera traffic from leaving the network
    for l in 0 1 2 3 4 5;
    do
        temp="60"
        tempc="$temp$l"
        camera200="fd00:200:3:$tempc::/56"
        camera300="fd00:300:3:${tempc}::/56"
        ip6tables -A FORWARD -s $camera200 -d fd00:200::3/48 -j DROP
        ip6tables -A OUTPUT -s $camera200 -d fd00:200::3/48 -j DROP
        ip6tables -A FORWARD -s $camera300 -d fd00:200::3/48 -j DROP
        ip6tables -A OUTPUT -s $camera300 -d fd00:200::3/48 -j DROP;
    done
