...
# Block packets with an IP corresponding to our network but coming from the outside
ip6tables -A INPUT -i pop200 -s fd00:200:2::/48 -j DROP
ip6tables -A INPUT -i pop200 -s fd00:300:2::/48 -j DROP
ip6tables -A FORWARD -i pop200 -s fd00:200:2::/48 -j DROP
ip6tables -A FORWARD -i pop200 -s fd00:300:2::/48 -j DROP

# Allow BGP
ip6tables -A INPUT -i pop200 -p tcp --dport 179 -j ACCEPT
ip6tables -A INPUT -i pop200 -p tcp --sport 179 -j ACCEPT
ip6tables -A OUTPUT -o pop200 -p tcp --dport 179 -j ACCEPT

# Block OSPF packet coming from/going outside the network
ip6tables -A INPUT -i pop200 -p 89 -j DROP
ip6tables -A FORWARD -i pop200 -p 89 -j DROP
ip6tables -A FORWARD -o pop200 -p 89 -j DROP
ip6tables -A OUTPUT -o pop200 -p 89 -j DROP

# Block DHCP from leaving or entering the network through pop200
ip6tables -A INPUT   -p udp -i pop200 -m multiport --dports 546,547 -j DROP
ip6tables -A FORWARD -p udp -i pop200 -m multiport --dports 546,547 -j DROP
ip6tables -A FORWARD -p udp -o pop200 -m multiport --dports 546,547 -j DROP
ip6tables -A OUTPUT  -p udp -o pop200 -m multiport --dports 546,547 -j DROP
...
ip6tables -A FORWARD -p ipv6-icmp -i pop200 -j DROP
...
ip6tables -A INPUT -j NFLOG --nflog-prefix "[DROP-INPUT]PYTH"
ip6tables -A FORWARD -j NFLOG --nflog-prefix "[DROP-FORWARD]PYTH"
ip6tables -A OUTPUT -j NFLOG --nflog-prefix "[DROP-OUTPUT]PYTH"
