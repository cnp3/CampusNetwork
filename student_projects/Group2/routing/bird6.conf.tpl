{# THIS FILE IS A TEMPLATE USED TO GENERATE ALL BIRD6 CONFIGURATION FILES FOR ALL ROUTERS #}

router id 255.0.0.{{router_id}}; # Sets the router unique ID

filter import_ospf_filter { # Filter used to determine which routes can be imported via OSPF
    {% if bgp.export %} {# bgp.export equals True if and only if this router is the default gateway for Internet #}
    {% else %}
...
filter export_ospf_filter { # Filter used to determine which routes can be exported via OSPF
    {% if bgp %} {# If this router handles BGP #}
...
protocol bgp eBGP { # BGP protocol
    ...
    neighbor {{ bgp.neighbor_ip }} as {{ bgp.neighbor_as }}; # Defining our neighbor
    source address {{ bgp.source_ip }};   # Use a non-standard source address
...
protocol ospf MyOSPF {
   ...
   area 0 { # Define a unique area (all routers belong to the same area 0)
	  ...
	  stubnet {{network}}; # defines the LANs of the router (stubnet because there are no OSPF nodes in them)
	  {% endfor %}

	  {% for neighbor in neighbors %}
      interface "{{ neighbor }}" { # new ospf neighbor
